...
@section('title', 'Liste des pays du monde')
...
request.onload = function() {
  ...
  if (request.status >= 200 && request.status < 400) {
    data.forEach(country => {
      ...
      $(".countries ").append("<div class='col-lg-4 col-md-6'>\
          <div class='card-box widget-user'>\
              <div>\
                  <img style='width: 20%;' src='"+country.flag+"' class='img-fluid' alt='country'>\
                  <div class='wid-u-info'>\
                      <h4 class='m-t-0 m-b-5 font-600'>"+country.name+"</h4>\
                      <a href='/countries/"+country.name.replace(' ', '+')+"'>\
                      <small class='text-warning'><b>Voir les utilisateurs vivant dans ce pays</b></small>\
                      </a>\
                  </div>\
              </div>\
          </div>\
